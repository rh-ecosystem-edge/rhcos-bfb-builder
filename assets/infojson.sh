...
BFB=/lib/firmware/mellanox/boot/default.bfb
bfver_out="$( bfver --file ${BFB} )"

CAPSULE="/lib/firmware/mellanox/boot/capsule/boot_update2.cap"

ATF_UEFI_VERSION=$(rpm -qf --queryformat "[%{VERSION}].[%{RELEASE}]" $CAPSULE)
...
if [ -e "/usr/opt/mellanox/mlnx-fw-updater/firmware/mlxfwmanager_sriov_dis_aarch64_41692" ]; then
    let odata_count++
